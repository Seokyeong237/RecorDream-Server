...
const socailLogin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (user === null) {
      return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.LOGIN_FAIL));
    }

    // fcmToken이 없거나 kakaoToken과 appleToken이 모두 없으면 에러
    if (!fcmToken || (!kakaoToken && !appleToken)) {
      return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.NULL_VALUE));
    }

    // firebaseUID와 kakaoAccessToken이 모두 있어도 에러
    if (kakaoToken && appleToken) {
      return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.ONE_TOKEN));
    }

    res.status(statusCode.OK).send(util.success(statusCode.OK, message.LOGIN_SUCCESS, user));
  } catch (err) {
    console.log(err);
